...
class CatinyUaaKafkaResourceIT
{
  ...
  void setup()
  {
    ...
    CatinyUaaKafkaResource kafkaResource = new CatinyUaaKafkaResource(kafkaProperties);
  ...
  {
    restMockMvc.perform(post("/api/catiny-uaa-kafka/publish/topic-produce?message=value-produce"))
  ...
  void consumesMessages() throws Exception
  {
    ...
    MvcResult mvcResult = restMockMvc.perform(get("/api/catiny-uaa-kafka/consume?topic=topic-consume"))
      .andExpect(status().isOk())
